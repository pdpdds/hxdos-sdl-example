...
const char *IMG_small_yummy = "data/s_yummy.bmp";
const char *IMG_big_yummy = "data/b_yummy.bmp";
const char *IMG_redg = "data/red.bmp";
const char *IMG_cyang = "data/cyan.bmp";
const char *IMG_pinkg = "data/pink.bmp";
const char *IMG_vulng = "data/vuln.bmp";
...
static Uint32 next_ghost_update_time;
...
void game()
{
	...
	next_ghost_update_time = next_time + 5000;
	while(!done)
	{		
		if(killed)
		{
		...
		if(SDL_PollEvent(&event)) //quit being a fucking CPU hog
		{
			switch(event.type)
			{
				case SDL_KEYDOWN:
		...
		if(next_ghost_update_time < next_time)
		{
			updategstat();
			next_ghost_update_time += 5000;
		}
